   ...
   public Node BuildTreeRoot()
    {
        ...
        return new DecoratorLoop(
            new Sequence(
                ...
                new LeafInvoke(()=>spidol.SetActive(true)),
                new DecoratorForceStatus(RunStatus.Success,
    ...
    protected Node StealIdol(GameObject agent)
    {
        return new DecoratorForceStatus(RunStatus.Success,
            new Sequence(
                new LeafInvoke(()=> Debug.LogError(agent+" "+ (GameObject.Find("PricelessIdol") != null))),
                new LeafAssert(()=>GameObject.Find("PricelessIdol")!=null),
                new GoToIdol(agent.GetComponent<SmartCharacter>(),GameObject.Find("IdolContainer").GetComponent<SmartIdol>()).PBT(),
