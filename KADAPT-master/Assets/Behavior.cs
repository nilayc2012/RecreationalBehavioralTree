...
public class Behavior : MonoBehaviour {
   ...
   public Node BuildTreeRoot()
    ...
    protected Node[] GetPoliceBehaviors()
    {
        ...
        {
            policeList.Add(new DecoratorLoop(new Sequence(new GoToCorner(nep.GetComponent<SmartCharacter>(),north.GetComponent<SmartCorner>()).PBT(), new GoToCorner(nep.GetComponent<SmartCharacter>(), east.GetComponent<SmartCorner>()).PBT())));
        }

        foreach (GameObject nsp in NSPolice)
        {
            policeList.Add(new DecoratorLoop(new Sequence(new GoToCorner(nsp.GetComponent<SmartCharacter>(), north.GetComponent<SmartCorner>()).PBT(), new GoToCorner(nsp.GetComponent<SmartCharacter>(), south.GetComponent<SmartCorner>()).PBT())));
        }

        foreach (GameObject ewp in EWPolice)
        {
            policeList.Add(new DecoratorLoop(new Sequence(new GoToCorner(ewp.GetComponent<SmartCharacter>(), east.GetComponent<SmartCorner>()).PBT(), new GoToCorner(ewp.GetComponent<SmartCharacter>(), west.GetComponent<SmartCorner>()).PBT())));
        }

        foreach (GameObject swp in SWPolice)
        {
            policeList.Add(new DecoratorLoop(new Sequence(new GoToCorner(swp.GetComponent<SmartCharacter>(), south.GetComponent<SmartCorner>()).PBT(), new GoToCorner(swp.GetComponent<SmartCharacter>(), west.GetComponent<SmartCorner>()).PBT())));
        }
