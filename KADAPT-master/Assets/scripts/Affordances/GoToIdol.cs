...
public class GoToIdol : Affordance {
	...
	public Node PBT(){
		...
		return (new Sequence(this.affordant.gameObject.GetComponent<BehaviorMecanim> ().Node_GoToUpToRadius (pos,2),new LeafInvoke(()=>GameObject.Find("PricelessIdol").SetActive(false)), this.UpdateAndPublish()));
	}
}
